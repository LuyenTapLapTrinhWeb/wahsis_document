/**DICH NGON NGU TRONG TABLE LIST KHI BẤM SEARCH THEO COMBOBOX LANGUAGE*/
$scope.$watch(
  $scope => {
...
/**DICH NGON NGU TRONG TABLE LIST KHI CHI BẤM LÁ CỜ*/

/* html
 */<td style="white-space: nowrap">{{ dichngonngu(item.status)}}</td>
/* js */
$scope.$watch(
  $scope => {
    return ($scope.abc = $filter("translate")("NOTE_FORMAT1"));
  ...
  () => {
    $scope.dichngonngu = (value) => {
      if (value === 0) {
        return $scope.abc = $filter("translate")("current1");
      } else if (value === 1) {
        return $scope.abc = $filter("translate")("complete1");
      } else if (value === 2) {
        return $scope.abc = $filter("translate")("incomplete1");
      } else {
        return "null";
      }
    }
